...
from googletrans import Translator  # Improved translation using googletrans
...
def translate_to_hindi(text):
    """Translate English text to Hindi using googletrans for better quality."""
    try:
        translator = Translator()
        # Split into 100-word chunks for better translation quality.
        chunks = chunk_text_by_words(text, chunk_size=100)
        hindi_chunks = []
        for chunk in chunks:
            translation = translator.translate(chunk, src='en', dest='hi')
            hindi_chunks.append(translation.text)
        return " ".join(hindi_chunks)
    except Exception as e:
